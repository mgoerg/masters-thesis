...
The theory of diffeological spaces is a generalization of the theory of diffeological spaces. It allows us to define differential forms on mapping spaces such as the path and the loop space of a manifold. These forms have the same algebraic operations of pullback, wedge product and exterior derivative as in the case of smooth manifolds. There is a de Rham map, which in nice cases, though not in general, provides a quasi-isomorphism to with the singular cohomology. There is also a fiber integration map on de Rham forms, which we construct for projections $M\times X\to X$ with closed oriented manifolds $M$. 
...
In this subsection we introduce the smooth singular cohomology and compare it to the singular cohomology, in preparation for the construction of the de Rham map in \cref{subsec:diffeological-de-rham}. 
...
\end{proposition}
\begin{proof}
    Let $\sigma\colon\Delta^n\to X$ be a smooth singular simplex and $\omega\in\Omega^*(X)$. The form $\sigma^*\omega$ is a differential form in the classical sense on $\Delta^n$ and it is well known that the de Rham map on smooth manifolds is a chain map, hence $\int_{\Delta^n}d\sigma^*\omega = \int_{\partial\Delta^n}\sigma^*\omega$ and thus $\int_{\Delta^n}d\omega = \int_{\partial\Delta^n}\omega$. 
...
Let $M$ be a smooth manifold. We want to equip the path space of $M$ with the structure of a diffeological space. A first attempt is to use the mapping space diffeology of maps from $I\to M$, as in \cref{prop:diffeology-mapping-space}. The smooth maps from $I$ to $M$ in the diffeological sense are the smooth maps in the classical sense. However we want to model the concatenation of paths $PM\times_M PM\to PM$, which for general smooth paths is not smooth. We also want to model the splitting map $PM\times I\to PM\times_M PM$ which splits a path into two paths at time $t$. To fix this issue, we work with piecewise smooth paths instead. 

\begin{definition}\label{def:diffeology-piecewise-smooth-paths} The \emph{diffeology on the space of piecewise smooth paths} \index{diffeology!on piecewise smooth paths} is given by defining a map $\alpha\colon U\to PM$ to be a plot if the adjoint map $\#\alpha\colon U\times I\to M$ is continuous and has the property that, for some partition $0=t_0<t_1<\dots<t_r=1$ of the unit interval $I$, the restriction $\#\alpha\mid_{U\times[t_i, t_{i+1}]}$ extends to a plot for $M$ on some open neighborhood for all $i$. 
\end{definition}

This construction is due to \cite{gugenheim1977chen}. Similar constructions apply to the based and free loop space $\Omega M$ and $LM$. More generally, let $G$ be a topological space which is the geometric realization of a graph and let $\Map(G, M)$ be the space of continuous maps $G\to M$ which are piecewise smooth along the edges of $G$. With a similar construction as in \cref{def:diffeology-piecewise-smooth-paths} one can define the \emph{diffeology on $\Map(G, M)$}. Denote by $C^0(G, M)$ the topological space of continuous maps from $G$ to $M$, with the topology of uniform convergence.

The results of \cref{sec:cochain-models} and \cref{sec:loop-models} depend on the assumption that $\Omega^*(\Map(G, M))$ computes the singular cohomology of $\C^0(G, M)$. These complexes are related through the sequence of chain maps. 
\begin{align}
...
The de Rham map $$\Omega^*(X) \to C_\infty^*(X)$$ is in general is not a quasi-isomorphism. Recall that the proof of the de Rham theorem in the case of smooth manifolds depends on structure of the manifold that is not available in the case of diffeological spaces, e.g.\ a locally finite covering with contractible spaces and contractible intersections. It appears to be folklore that the de Rham map is an isomorphism in the case of $LM$, but we are unable to give a proof. 
...
\begin{construction} [Euclidean Fiber Integral]
    Let $U\subset \R^n, V\subset\R^m$ be open and $\omega\in\Omega^*(U\times V)$, then $\omega$ is a sum of forms $f_{I, J} du_{i_1}\wedge\dots \wedge du_{i_k}\wedge dv_{j_1}\wedge\dots\wedge dv_{j_l}$ with $I=(i_1,\dots, i_k), J=(j_1,\dots, j_l)$ for unique smooth functions $f_{I,J}\in C^\infty(U\times V)$. We say that the fiber integral of $\omega$ is well-defined if for all $u\in U$, the integral $$\int_V f_{I,J}(u, v) dv_{j_1}\wedge\dots\wedge dv_{j_l}$$ converges for all multi-indices $I, J$ with $l = \abs{J} = \dim V$ and yields a smooth function $U\to \R$. In this case, we define the \emph{fiber integral} of $\omega$ to be the form $\int_V \omega\in\Omega^*(U)$ given at $u\in U$ by
    $$u\mapsto \left(\int_V \omega\right)_u = \sum du_{i_1}\wedge\dots \wedge du_{i_k} \left(\int_V f_{I, J}(u, v) dv_{j_1}\wedge\dots\wedge dv_{j_l}\right).$$
    The integral is formally set to $0$ if $l\neq n=\dim V$. 
\end{construction}
For example, a form $\omega\in \Omega^*(U\times V)$ has compact vertical support if for all $u$, the set $\{v\in V\times V\mid \omega|_{(u, v)}\neq 0\}\subset V$ is contained in a compact subset of $V$. Fiber integration then yields a map $$\Omega^*_{cv}(U\times V)\to \Omega^{*-\dim V}(U),$$ where $\Omega^*_{cv}(U\times V)$ denotes the forms with compact support in $V$ direction. 
...
\begin{remark}\label{rmk:fiber-int-cpt}
    ...
    $$u\mapsto \left(\int_K \omega\right)_u = \sum du_{i_1}\wedge\dots \wedge du_{i_k} \left(\int_K f_{I, J}(u, v) dv_{j_1}\wedge\dots\wedge dv_{j_l}\right).$$
...
\end{proposition}
\begin{proof}
    For the first part, we may take $F=\Delta^k, X=\Delta^l$ without loss of generality. The Eilenberg-Zilber map gives a partition of the product $\Delta^{k}\times\Delta^l$ via smooth maps $\Delta^{k+l}\to\Delta^k\times\Delta^l$ with disjoint images except on a zero set, hence we conclude. The remaining statements are implied by \cref{lem:fiber-int-euclidean}. 
\end{proof}
...
\begin{construction}\label{constr:loop-intersection-via-configuration}
    We define the intersection map $i_{LM\times_M LM}^!$ of the inclusion $i_{LM\times_M LM}\colon LM\times_M LM\to LM\times LM$ as the following composition 
\begin{equation}
    \begin{tikzcd}
...
\begin{align*}
    H^*(LM)\otimes H^*(LM) \xfrom H^*(LM\times LM) \xfrom{i^!} H^{*-n}(LM\times LM|_M) \xfrom H^{*-n}(LM),
\end{align*}
...
\begin{proof}
    ...
    Thus we have shown that the following commutes in $D(k)$: 
