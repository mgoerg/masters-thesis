...
By a result due to Quillen \cite{quillen1969rational}, the study of the homotopy of simply connected topological spaces is equivalent to the study of the homotopy properties of simply connected differential graded commutative algebras. Given a specific simply connected manifold $M$, one may construct other spaces from $M$, such as the free loop space $LM = C^0(S^1, M)$ and the configuration spaces $\Conf_M(n)$ of $n$ ordered distinct points in $M$. Given a differential graded algebra $A$ which computes the cohomology of $M$, such as the de Rham algebra $\Omega^*(M)$ or the singular cochain algebra $C^*(M)$, one may ask if it is possible to construct differential graded algebras which compute the cohomology of spaces such as $LM$ and $\Conf_M(n)$. In addition, one may ask to translate maps between spaces to maps of algebras. Early examples of this are Adams' work on the based loop space $\Omega M$ and the cobar construction \cite{adams1956cobar} and Chen's work \cite{chen1977iterated} on the bar construction and the free loop space $LM$. In the 1980s, Jones showed that there is an isomorphism between the cohomology of the loop space and the Hochschild cohomology of $C^*(M)$. 

We will apply such algebraic models in the context of String Topology. String Topology was initiated by Moira Chas and Dennis Sullivan in \cite{chas1999string} and it is concerned with the study of algebraic operations on the homology of the free loop space $LM=C^0(S^1, M)$ of a closed oriented manifold $M$. They introduced a product operation $$\cdot\colon H_*(LM)\otimes H_*(LM)\to H_{*-\dim M}(LM)$$ and a rotation operator $\Delta\colon H_*(LM)\to H_{*+1}(LM)$. Another well known operation of String Topology is the loop coproduct $H_{*-n+1}(LM)\to H_*(LM)\otimes H_*(LM)$, a degree $n-1$ coproduct.



According to \cite[Sec. 1]{naef2022string}, String Topology is at the intersection of several fields of mathematics, including the following: 
\begin{itemize}
    \item There is an interpretation in Symplectic Geometry, the simplest instance of this is that the Floer homology of the cotangent bundle $T^*M$ is isomorphic to $H_*(LM)$ and the loop product has an interpretation in Floer homology, cf.~\cite{abbondandolo2010floer} and for a more general picture~\cite{cieliebak2009role}. 
    \item The homology of $LM$ and string operations can be used to study closed geodesics on Riemannian manifolds via Morse theory on the energy functional. Gromoll and Meyer \cite{gromoll1969periodic} showed for a closed simply connected manifold $M$, if the sequence of Betti numbers of $LM$ is unbounded, then there exist infinitely many geometrically distinct periodic geodesics in $M$. More recently, the loop product has been used to study closed geodesics as well, compare \cite{Goresky_2009}. 
    \item Families of higher operations have been introduced on $H_*(LM)$. This leads to the study of properads and 2-dimensional field theories, cf.~\cite{godin2007higher}, \cite[I.2, I.3]{cohen2006string}.
\end{itemize}
...
The construction of $i_!$ is involved and there are several approaches, for example using the Thom isomorphism \cite{cohen2006string}, \cite{hingston2017product} or via transversality \cite{chataur2005bordism}.

\subsection{Results and Outline}
We follow \cite{naef2019string}. Our main goal is to give a clean presentation of the proof that the loop product and BV operator induce a co-BV algebra on the cohomology $H^*(LM)$. 


Let $G$ be a finite graph and $M$ a simply connected closed manifold. Then there is a space associated to $G$ which we also denote by $G$ and we can consider the mapping space $\Map(G, M) = C^0(G, M)$, with the topology of uniform convergence along the edges of $G$. Let $A$ be a differential graded algebra which models $M$, such as $C^*(M)$ or $\Omega^*(M)$. In \cref{sec:cochain-models} we construct a chain complex $A^{\otimes G}$. % whose cohomology is isomorphic to the cohomology of $\Map(G, M)$.
We will show the following in \cref{subsec:graph-models}: 
\begin{theorem}\label{thm:intro-graph-model}
    \begin{enumerate}
        \item The assignment $(A, G)\mapsto A^{\otimes G}$ yields a contravariant functor in $A$ which preserves quasi-isomorphisms and a covariant functor in $G$ 
        ...
        \begin{enumerate}
            ...
            \item In coefficients in a field of characteristic $0$, there exists a natural zigzag of quasi-isomorphisms $$C^*(M)^{\otimes G}\to\dots\from C^*(\Map(G, M))$$
        \end{enumerate}
    \end{enumerate}
\end{theorem}
This construction generalizes the isomorphism between the Hochschild cohomology and the loop space cohomology $$HH_*(A, A)\iso H^*(LM)$$ and the bar construction $\BC(A, A, A) \quiso C^*(PM)$. The map $I$ is a generalization of the iterated integral map due to Chen~\cite{chen1977iterated}. The main tool for this construction is the derived tensor product $M\otimes^L_A N$ of two differential graded modules over a differential graded algebra $A$. This can be computed by the two sided bar construction $B(M, A, N)$, which we introduce in \cref{subsec:bar_construction} and \cref{subsec:bar-constr-derived}. 
...
In \cref{sec:string-ops-via-cfg-spaces} we introduce a construction of the umkehr map $i_!\colon H_*(LM\times LM)\to H_{*-n}(LM\times_M LM)$ due to \cite{naef2019string}, and the cohomlogical analogue $i^!_{LM}\colon H^{*-n}(LM\times_M LM)\to H^*(LM\times LM)$. This construction relies on the compactification of configuration spaces. We justify that this construction yields the same umkehr map as the classical construction using tubular neighborhoods and the Thom isomorphism. An additional result is the following lemma, cf~\cref{lem:loop-intersection-map-product}, which shows that the umkehr map $i^!_{LM}$ can be constructed algebraically from the corresponding umkehr map $i^!\colon H^{*-n}(M)\to H^*(M\times M)$. 
\begin{lemma}
...
In \cref{sec:loop-models} we give models of the loop operations which only depend on a differential graded algebra $A$ which computes the cohomology of $M$. In \cref{subsubsec:frobenius-algebra-models} we recall that, by \cite{lambrechts2008poincare}, for a simply connected oriented closed manifold $M$, there is a Frobenius algebra $A$ which is quasi-isomorphic to $\Omega^*(M)$. The product of $A$ takes the role of the wedge product, while the degree $n$ coproduct is the umkehr map of the diagonal. It is well known that there is the structure of a co-BV algebra on the Hochschild homology $HH_*(A, A)$ if $A$ is a Frobenius algebra. We will prove the cohomological analogue of the statement that the loop product and BV operator form a BV algebra. In cohomology one has a coproduct and the so called co-BV operator, which satisfy analogues of the BV relations. The following is shown in \cref{thm:bv-iso}
\begin{theorem*}
    The isomorphism $HH_*(A, A)\iso H^*(LM)$ preserves the loop product and the (co-)BV operator. In particular the loop product and the co-BV operator form a co-BV algebra on $H^*(LM)$. 
\end{theorem*}

Naef and Willacher frame their results using the de Rham complex in \cite{naef2019string}, even on infinite dimensional spaces such as the path space $PM$ and loop space $LM$. In \cref{sec:diffeological} we explain the framework of diffeological spaces, which allows us to define de Rham forms on more general spaces than manifolds. The arguments in \cite{naef2019string} are possible because the de Rham complex $\Omega^*(X)$ of a diffeological space $X$ has the following properties: 
\begin{itemize}
    ...
    \item It is quasi-isomorphic to $C^*(X)$ for nice spaces (though not in general). 
\end{itemize}
The singular cochain complex almost satisfies these requirements as well, as the analog of the fiber integral is the slant product, see \cref{subsec:slant_product}, but the product is only homotopy commutative. 
In the main part of the thesis, we develop the results of \cite{naef2019string} on de Rham forms, as well as an analogue on the singular cochain complex. With de Rham forms we can often construct maps on the chain level which we cannot construct as easily on the singular complex. On the singular complex we fill these gaps by working on cohomology or in the setting of derived categories instead; this often leads to shorter proofs, but we do not have the benefit of working at the chain level.
...
% \end{align*}
% \end{construction}
% Similarly, using the loop coproduct one can construct the string cobracket. 
...
In this subsection we consider the diffeology on path spaces and similar constructions of a smooth manifold $M$. We exhibit a diffeological structure on the path space that is suitable for our computation and discuss the relation between its de Rham cohomology and singular cohomology.

A first attempt for a diffeology on the path space $PM$ of $M$ is to use the mapping space diffeology of maps from $I\to M$, as in \cref{prop:diffeology-mapping-space}. The smooth maps from $I$ to $M$ in the diffeological sense are the smooth maps in the classical sense. However we want to model the concatenation of paths $PM\times_M PM\to PM$, and in general the concatenation of two smooth paths is not smooth. 
...
Denote by $C^0(G, M)$ the topological space of continuous maps from $G$ to $M$, with the topology of uniform convergence. The results of \cref{sec:cochain-models} and \cref{sec:loop-models} are stated using the de Rham complex in \cite{naef2019string} and as such depend on the assumption that $\Omega^*(\Map(G, M))$ computes the singular cohomology of $C^0(G, M)$. These complexes are related through the sequence of chain maps. 
\begin{align}
...
\begin{theorem} \label{thm:diffeological-quiso-singular}
    If $\Map(G, M)$ has the structure of a smooth CW complex, then the de Rham comparison map 
    \begin{align*}
        \Omega^*(\Map(G, M)) \to C_\infty^*(\Map(G, M)) 
    \end{align*}
    is a quasi-isomorphism. The maps 
    \begin{align*}
        C_\infty^*(\Map(G, M)) \from C^*(\Map(G, M)\from C^*(C^0(G, M))
    \end{align*}
    are always quasi-isomorphisms. 
\end{theorem}

We are unaware of a proof in the literature that the space $\Map(G, M)$ is a smooth cw complex or that the de Rham comparison map is a quasi-isomorphism on these spaces. This assumption is required to proceed with the arguments in \cite{naef2019string}, as we outline them in \cref{sec:cochain-models} and \cref{sec:loop-models}. We thus make the following assumption. 
...
\end{assumption}
Note that this assumption is only required in \cref{sec:cochain-models} and \cref{sec:loop-models} for statements involving the de Rham complex. We develop parallel arguments for the singular cochain complex, these do not require this assumption. 
...
\end{proof}
This shows the second part of \cref{thm:diffeological-singular-cohomology-iso}.

\begin{lemma}
    The inclusions $C^\infty(I, M)\subset C^\infty_{pw}(I, M)\subset C^0(I, M)$ are continuous and induce homotopy equivalences, where $C^\infty_{pw}$ is the space of piecewise smooth paths. 
\end{lemma}
See \cite[Sec 1]{hingston2017product}. From this we conclude the same for $\Map(G, M)$ and thus the third part of \cref{thm:diffeological-singular-cohomology-iso}.
% \begin{lemma}
...
%     By the discussion in \cite{felix2012rational}[ch. 11], the map $A_{dR}(S^\infty_*(X))\from \APL(S^\infty_*(X))$ is a quasi-isomorphism. By the cite{kuribayashi2020simplicial}[theorem 2.4], the map $\Omega^*(X)\to A_{dR}(S^\infty_*(X))$ is a quasi-isomorphism if and only if the map $\Omega^*(\Map(X)) \to C_\infty^*(\Map(X))$ is a quasi-isomorphism. The map $\APL(S^\infty_*(\Map(G, M)))\from \APL(S_*(\Map(G, M)))$ is a homotopy equivalence by a similar argument as in \cref{prop:comparison-singular-smooth-cohomology}. The map $\APL(\Map(G, M))\to \APL(C^0(G,M))$ is a quasi-isomorphism if and only if the map on the singular cochain complex is. 
...
\end{definition}
The differential is well-defined on $B$. By \cref{lem:bar-normalization}, the quotient map $\BC(A, A, A)\to \nBC(A, A, A)$ is a quasi-isomorphism. 
...
\begin{lemma}
    ...
    \[\begin{tikzcd}
        ...
        C^*(\Map(E\times I, M)) \uar[swap, "\quiso"] & \lar  C^*(\Map(E\times \{0, 1\}, M)) \rar \uar[swap, "\quiso"] &  C^*(\Map(V, M))\uar[swap, "\quiso"]
    \end{tikzcd}\]
\end{lemma}
...
\begin{enumerate}[(1)]
    \item Construct a quasi-isomorphism $C^*(M\times M, \cConf_M(2))\quiso C^*(M, UTM)$
    \item Construct a Thom map $C^{*-n}(M)\to C^*(M, UTM)$.
\end{enumerate}
...
\begin{lemma}\label{lem:M-cofib-hteq}
    ...
    There is a quasi-isomorphism $$C^*(M\times M, \cConf_M(2))\xto{\quiso} C^*(M, UTM).$$
\end{lemma}
\begin{proof}
    The first part is shown above, the second part follows using the zigzag from \cref{rmk:cofib-cone}.
\end{proof}

The spaces are in fact homotopy equivalent to the Thom space $DTM / UTM$, where $DTM$ is the unit disk bundle in $TM$: the inclusion $UTM\to DTM$ is a cofibration, so the homotopy cofiber is given by the quotient in this case. 
...
\begin{definition}\label{constr:thom-map-cfg-spaces}
...
is given by $c\mapsto (c\cupp u|_{DTM}, 0)$, this is a chain map since $u|_{DTM}\in C^*(DTM)$ vanishes in $C^*(UTM)$ (the inclusion of $C^*(DTM)$ into the mapping cone is not a chain map by itself). 
...
\begin{remark}
    ...
    The class $u|_M$ represents the Euler class in $H^n(M)$. However $u|_M$ cannot be chosen arbitrarily inside its cohomology class and it does not vanish even for manifolds with vanishing Euler class: in fact, $Th$ is a quasi-isomorphism. 
\end{remark}
...
\begin{proof}
   ...
   \begin{equation}
    ...
    \end{tikzcd}\]
    The center square is justified by the above. The triangle on the right is a more concise version of Diagram \cref{diag:thom-map-construction}.
...
\begin{lemma}\label{lem:thom-map-LM-equal}
    ...
    \begin{tikzcd}
        ...
        \uar[swap, "\quiso"] C^*(LM\times LM)\otimes^L_{C^*(M\times M)} C^*(M)[-n] \rar["\id\otimes Th"] &  C^*(LM\times LM)\otimes^L_{C^*(M\times M)} \cone(C^*(UTM)\from C^*(M)) \uar[swap, "\quiso"]
    \end{tikzcd}
    \end{equation}
\end{lemma}
\begin{proof}
...
\begin{corollary}\label{thm:loop-products-identity}
    ...
    The two constructions of the derived intersection map $i^!_{LM}\colon C^*(LM\times LM) \from C^{*}(LM\times_M LM)[-n]$ from \cref{subsec:loop-product-classical} and \cref{constr:loop-intersection-via-configuration} are equal as maps in $D(Ch(\R))$. In particular they are the same on cohomology.
\end{corollary}


% \begin{remark}
%     One may attempt to extend the methods in the proof of \cref{thm:intersection-product-via-cfg-spc} to show that the two definitions of the intersection map are equal. Instead we will show in \cref{lem:loop-intersection-map-product} that the $i_{LM\times_M LM}^!$ on $LM$ is induced by the intersection map $i^!$ on $M$ in an algebraic way and conclude that the two intersection maps on $LM$ are equal from \cref{thm:intersection-product-via-cfg-spc}.
% \end{remark}
...
\end{theorem}
\begin{proof}
    We can consider the two squares separately. For the left square, recall from \cref{lem:loop-intersection-map-product} the commutative diagram 
    \begin{equation}
        \begin{tikzcd}
            C^*(LM\times LM) & \ar[l, swap, "i^!_{LM}"] C^{*}(LM\times_M LM)[-n]  \\
            \uar[swap, "\quiso"] C^*(LM\times LM) \otimes^L_{C^*(M\times M)} C^*(M\times M) & \uar[swap, "\quiso"]\lar[swap, "\id\otimes i^!"] C^*(LM\times LM) \otimes^L_{C^*(M\times M)} C^*(M)[-n]
...
Let $K$ be a field, $A$ be a unital differential graded algebra over $K$, $M$ a right dg $A$ module and $N$ a left $A$ dg module. The chain complex $M\otimes A^{k}\otimes N$ is a chain complex with the usual tensor product differential 
\begin{align*}
...
\end{align*} 
Since this is induced by a simplicial object, it follows that $d_1\comp d_1 = 0$ and since the face maps are chain maps with respect to $d_0$, the two differentials commute: $d_1 \comp d_0 = d_0\comp d_1$. 
...
\begin{theorem}\label{thm:bar_resolution}
    ...
    The canonical map $\BC(M, A, A)\to M$ given by the product map $M\otimes A\to M$ for $n=0$ and $0$ otherwise, is a quasi-isomorphism of chain complexes. 
\end{enumerate}
\end{theorem}
\begin{proof}
...
We will show that homotopy pushouts can be computed using cofibrations. We quickly recall cofibrations here. For a more in-depth introduction we refer to \cite[Ch. 6]{may2011more}. 
...
We caution that the following two theorems do not hold in general model categories, even though they appear like formal results. 
