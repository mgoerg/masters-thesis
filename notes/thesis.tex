...
\section*{Abstract}
String topology is the study algebraic structures on the homology of the free loop space and related spaces. The goal of this thesis is to provide two constructions of the loop product and coproduct on a simply connected manifold $M$. The first construction uses the Fulton-MacPherson compactification of the configuration space of $n$ ordered points. The second construction is purely algebraic and shows that the intersection map in the definition of the loop product is algebraically a lift of the intersection product on $M$. This will allow us to give an algebraic proof that the cohomology of the free loop space has the structure of a co-Batalin-Vilkovisky algebra. Starting with a differential graded algebra model $A$ of $M$, we construct explicit cochain complexes which model mapping spaces from graphs into $M$, such as the path space, the free loop space and the figure eight space of $M$. A special case is the well known result that the loop space cohomology is isomorphic to the Hochschild homology. Our main tool is a theorem known as the pullback-pushout lemma, originally due to Eilenberg and Moore, which shows how one can compute the cohomology of a pullback of spaces using the derived tensor product. We also model maps between mapping spaces on our chain models, and as an application we can show that the isomorphism $HH_*(A)\iso H^*(LM)$ preserves the co-BV operations, which shows that the co-BV relations hold on $H^*(LM)$.
...
\begin{itemize}
    ...
    \item Families of higher operations can be defined on $H_*(LM)$. This leads to the study of properads and 2-dimensional field theories, cf.~\cite{godin2007higher}, \cite[I.2, I.3]{cohen2006string}.
\end{itemize}
...
The construction of $i_!^{LM}$ is involved and there are several approaches, for example using the Thom isomorphism \cite{cohen2006string}, \cite{hingston2017product} or via transversality \cite{chataur2005bordism}.

\subsection{Results and Outline}
The main goal of this thesis is to give a clean presentation of the construction of the loop product and BV operator in \cite{naef2019string}. 
...
\begin{definition}
    ...
    \begin{enumerate}
        \item A \emph{Batalin-Vilkovisky (BV) algebra}\index{BV algebra} on $V$ is given by a degree $0$ product $\colon V\otimes V\to V$ and a degree $-1$ operator $\Delta\colon V\to V$, such that 
        \begin{itemize}
            ...
            \begin{align*}
                0 = &\Delta(abc) - \Delta(ab)c - (-1)^{\abs{a}} \Delta(bc) - b (-1)^{(\abs{a}+1)\abs{b}} b\Delta(ac) \\ &+ \Delta(a) bc + (-1)^{\abs{a}}a\Delta(b) c + (-1)^{\abs{a}+\abs{b}}ab\Delta(c)
            \end{align*}
    ...
    \end{enumerate}
\end{definition}
For a graded vector space $V$ and $k\in \Z$, we denote by $V[k]$ the graded vector space with $V[k]_n = V_{k+n}$. If $V$ is a chain complex, then we define a differential on $V[k]$ by $d_{V[k]} = (-1)^k d_V$.
...
\begin{center}
    ...
    \end{tikzcd}
\end{center}
We will use the following well known result (cf.\ \cref{thm:pullback-pushout}):
\begin{theorem}
...
\end{theorem}
Here we use cohomology with coefficients in a field of characteristic $0$.
...
\begin{theorem}
    ...
    \end{center}
\end{theorem}
The maps $i^!$ and $i^!_{LM}$ are induced by zigzags of chain maps, in \cref{subsec:derived} we recall that in the derived category of chain complexes one can interpret these as maps instead. 



In \cref{sec:loop-models} we give models of the loop operations which only depend on a differential graded algebra $A$ which computes the cohomology of $M$. Recall that for an oriented closed manifold $M$, the intersection coproduct $H^{*-n}(M)\to H^*(M)\otimes H^*(M)$ together with the cup product, induces a Frobenius algebra structure on $H^*(M)$. In \cref{subsubsec:frobenius-algebra-models} we recall that by \cite{lambrechts2008poincare}, if $M$ is additionally simply connected, there is a Frobenius algebra $A$ which is quasi-isomorphic to $\Omega^*(M)$ and which induces the Frobenius algebra structure on cohomology. Using this chain level intersection map, we find that we have chain level models for all the maps in the construction of the loop product in \cref{sec:string-ops-via-cfg-spaces}, using the results from \cref{sec:cochain-models}. This allows us to show that the isomorphism $HH_*(A, A)\iso H^*(LM)$ preserves the loop product (cohomology coproduct). A similar analysis shows that the BV operators are preserved. In particular this shows the co-BV relation on $H^*(LM)$. 
...
We use piecewise smooth paths instead. 
\begin{definition}
    We denote by $C^\infty_{ps}(I, M)$ the space of piecewise smooth paths in $M$. We denote by $C^\infty_{ps, b}(I, M)$ be the spaces of piecewise smooth paths in $M$ resp.~piecewise smooth paths with bounded derivatives on $M$, that is, each derivative is a bounded function on its domain of definition.
\end{definition}
\begin{definition}
...
Analogously, one can equip the space $PM\times_M PM$ with a diffeological structure as a pullback.
\begin{lemma}
    The composition map $c\colon PM\times_M PM\to PM$ and the splitting map $s\colon I\times PM\to PM\times_M PM$ restrict to smooth maps of diffeological spaces.
\end{lemma}
\begin{remark}
    We warn that the circle action $LM\times S^1\to LM$ and reparametrization maps $PM\to PM$ via piecewise smooth maps $I\to I$ are not continuous with respect to the topology that we have chosen. 
\end{remark}
...
\end{definition}
\begin{remark}
    In the following sections, the symbols $LM$ and $PM$ will be used both for the space of continuous maps and the space of piecewise smooth maps with bounded derivative. It will usually be clear from context which one is used: if we are working with the chain complex $C^*$ then it is the former, if we are working with the complex $\Omega^*$ then it is the latter. 
\end{remark}
...
\begin{theorem} \label{thm:diffeological-quiso-singular}
    \begin{enumerate}[(1)]
    \item If $\Map(G, M)$ has the structure of a smooth CW complex, then the de Rham comparison map 
    \begin{align*}
        \Omega^*(\Map(G, M)) \to C_\infty^*(\Map(G, M)) 
    \end{align*}
    is a quasi-isomorphism. 
    \item The following map is a quasi-isomorphism: $C_\infty^*(C^\infty(G, M)) \from C^*(C^\infty(G, M)$
...
\begin{assumption}\label{assumption}
    ...
    \begin{align*}
        \Omega^*(\Map(G, M)) \to C_\infty^*(\Map(G, M)) 
    \end{align*}
    is a quasi-isomorphism. We further assume that the map $$C_\infty^*(\Map(G, M)) \from C^*(\Map(G, M)$$ is a quasi-isomorphism. 
\end{assumption}
...
\begin{lemma}\label{lemma:comparison-singular-smooth-cohomology}
    If $M$ is a smooth manifold without boundary, then for finite $F\subset I$, the maps $$C^\infty_*(C^\infty_F(G, M)) \to C_*(C^\infty_F(G, M)),  \qquad  C^*(C^\infty_F(G, M)) \to C_\infty^*(C^\infty_F(G, M))$$ are quasi-isomorphisms.
\end{lemma}
\begin{proof}
    ...
    % Recall that $C^\infty_{ps, b}(I, M)$ is the filtered colimit of $C^\infty_{F, b}(I, M)$ over all finite $F\subset I$, both as a topological and as a diffeological space. One can show that for each singular simplex $\sigma\colon \Delta\to C^\infty_{ps, b}(I, M)$ there is some $F$ such that $\sigma$ maps into $C^\infty_{F, b}(I, M)$, see \cite[Prop. 5.6]{stacey2017smooth}. Thus, the singular chain complexes are also filtered colimits over all finite subsets $F\subset I$. Since the filtered colimit of a quasi-isomorphism is again a quasi-isomorphism, this shows that the map of the singular chain complexes
    % $$C^\infty_*(C^\infty_{ps, b}(I, M)) \to C_*(C^\infty_{ps, b}(I, M))$$
    % is a quasi-isomorphism. The statement on the cochain complex follows.
...
\end{remark}
    ...
    For an inclusion between spaces $f\colon X\injto Y$, the map $f^*\colon C^*(Y)\to C^*(X)$ is surjective and the natural map $C^*(Y, X)\to \cone(f^*)$ is a quasi-isomorphism. If $f$ is a general continuous map, then $f^*$ is in general not surjective and there is no long exact sequence in cohomology for $\ker f^*$.
...
\begin{enumerate}[(1)]
    \item Construct a quasi-isomorphism $C^*(M\times M, \cConf_M(2))\quiso C^*(M, UTM) = \cone(C^*(M)\to C^*(UTM))$
...
\end{corollary}
\begin{proof}
    For any Frobenius algebra $A$, one can show algebraically that operations $\Delta$ and $B$ on the Hochschild complex $CH_*(A, A) = \BC(A, A, A)\otimes_{A^{\otimes 2}} A$ satisfy the co-BV relations in $D(Ch)$, see theorem 5.4 in \cite{abbaspour2015algebraic} or theorem 1 of \cite{tradler2008bvalg}.
...
We remark that the Thom isomorphism is the umkehr map of the zero section, seen as a map $s\colon (B, \emptyset)\to (E, E_0)$, in the case where we have defined umkehr maps \footnote{More precisely, one has to assume that that $B$ is a closed oriented manifold and $E$ is an oriented vector bundle over $B$. For technical reasons, one has to replace the pair $(E, E_0)$ with the pair $(DE, UE)$ of the unit disk and unit sphere bundle of $E$. }. The Thom class is Poincaré dual to the zero section $s_*[B]\in H_n(E, E_0)$, cf.~\cite[Lemma 4.1]{hutchings2011cup}. As the Thom isomorphism can be constructed in a more general setting, without the requirement of Poincaré duality, it is used to construct umkehr maps in more general settings. 
