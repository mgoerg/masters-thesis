...
\begin{lemma}\label{lem:loop-intersection-map-product}
    ...
    \begin{center}
    \begin{tikzcd}
        C^*(LM\times LM) & \ar[l, swap, "i^!_{LM\times_M LM}"] C^{*}(LM\times_M LM)[-n]  \\
...
\begin{theorem}\label{thm:loop-product-derived}
    ...
    \begin{center}
    \begin{tikzcd}
        C^*(LM)\otimes C^*(LM) & \lar[swap, "i^!_{LM\times_M LM}"] C^*(LM\times_M LM)[-n] & \lar[swap, "c"] C^*(LM)[-n] \\
