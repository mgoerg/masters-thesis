...
We begin by outlining the definition of diffeological spaces in \cref{subsec:diffeological}. We then discuss how the cohomology of the de Rham complex of a diffeological space with a compatible topology relates to the cohomology of the original space in \cref{subsec:diffeology-cohomology} and \cref{subsec:diffeological-de-rham}. In \cref{subsec:diffeological-de-rham} we discuss the fiber integration map for projections $M\times X\to X$ smooth manifolds $X$. Finally we apply the theory to the case of mapping spaces such as $PM$ and $LM$. 
...
\begin{definition}
    ...
    \begin{enumerate}
        ...
        \item its \emph{smooth simplicial set} $S_*^\infty(X)$ which consists of all smooth simplexes $\Delta^n\to X$ for all $n$. By a smooth simplex we mean that there is a smooth map from a neighborhood of $\Delta^n$ to $X$. 
    \end{enumerate}
...
\end{definition}
The following lemma gives a sufficient condition for the smooth and continuous homology to be identical.
\begin{lemma}\label{thm:diffeological-singular-cohomology-iso}
    Let $X$ be a topological diffeological space such that for every continuous simplex $\sigma\colon \Delta^n\to X$ with smooth $(n-1)$-faces, $n\geq 1$, there exists a continuous homotopy $h\colon \Delta^n\times I\to X$ relative to the boundary $\partial\Delta^n$ from $\sigma$ to a smooth $n$-simplex of $X$. Then the canonical map $$C^\infty_*(X) \to C_*(X)$$ is a chain homotopy equivalence. Hence also the map $C^*(X) \to C^*_\infty(X)$ is a chain homotopy equivalence.
\end{lemma}
This theorem is stated in \cite{chen1977iterated}. To prove this one can proceed as in the case of manifolds, cf.\ \cite{parkproof}: one constructs a converse map $S(X)\to S^\infty(X)$ inductively by mapping each continuous simplex to a fixed smooth approximation, such that the approximations are compatible at the boundary. The topological homotopies of the approximation maps then yield a chain homotopy which witnesses that the two chain maps are homotopy inverse.

For smooth manifolds one can show that the hypothesis of the lemma is satisfied using Whitney's Approximation theorem, as in \cite{parkproof}. We state the Whitney approximation theorem here, as it is given in \cite[Theorem 6.26]{lee2003introduction}.
...
For any smooth simplex $\sigma\colon \Delta^n\to X$ and an $n$-form $\omega\in\Omega^n(X)$, one can define its integral $\int_{\sigma}\omega = \int_{\Delta^n}\sigma^*\omega$.
\begin{definition}
...
\end{definition}
$\sigma^*\omega\in\Omega^*(\Delta^n)$ is a differential form in the sense of smooth manifolds and since $\sigma$ extends smoothly near $\Delta^n$, the integral is a well-defined real number.
...
Let $M$ be a smooth manifold. Then the \emph{diffeology on the smooth path space} $PM$ of $M$ is the mapping space diffeology of $PM$. Unwrapping the definitions, a map $U\to PM$ is a plot if its adjoint $I\times U\to M$ is a map of smooth manifolds. For the free loop space $LM$ and the based loop space $\Omega M$ we use the analogous definition via the adjoint map $S^1\times U\to M$. Note that to satisfy the covering axiom, we have to use the space of smooth paths resp.\ loops in this context. \index{diffeology!on path spaces}

There is something more to be done: we are also interested in spaces such as $LM\times_M LM$, the space of figure eights in $M$, and we need the concatenation map $LM\times_M LM\to LM$, the projection to each factor $LM\times_M LM\to LM$ and the splitting map $I\times PM\to PM\times PM$. The concatenation of smooth paths is generally not smooth. One way to deal with this is to require loops to be constant near $t=0$. More generally, if we consider mapping spaces of graphs into $M$, we can also require that the path along each edge is constant near the vertices. This solves most of the issues except for the splitting map, which splits paths at an arbitrary point, not just at the vertices. 

As an alternative to this, one can use piecewise smooth paths.
\begin{definition} The \emph{diffeology on the space of piecewise smooth paths} \index{diffeology!on piecewise smooth paths} is given by defining a map $\alpha\colon U\to PM$ to be a plot if the adjoint map $\#\alpha\colon U\times I\to M$ has the property that, for some partition $0=t_0<t_1<\dots<t_r=1$ of the unit interval $I$, the adjoint $\#\alpha\mid U\times[t_i, t_{i+1}]\to M$ extends to a plot for $M$ on some open neighborhood for all $i$ and $\#\alpha$ is continuous. 
\end{definition}
...
%$\Omega^*(Y\times M)\to\Omega^{*-\dim M}(Y)$ for a closed oriented manifold $M$ and an integration map 
...
% \begin{construction}[Fiber Integral $\Omega^*(M\times Y)\to\Omega^{*-\dim M}(Y)$]
...
% Let $Y$ be a diffeological space, then one can construct the \emph{fiber integral}\index{fiber integral!on diffeological spaces} $$\int_M\colon\Omega^{*+\dim M}(Y\times M)\to \Omega^*(Y)$$ by mapping $\{\alpha_P\}\mapsto \{\int_M\alpha_P\}$. This satisfies the same invariance properties as in the lemma.
% \end{construction}

\begin{construction}[Fiber integral over smooth singular chains]
    If $X$ and $Y$ are diffeological spaces, $\sigma\colon\Delta^n\to X$ is a smooth simplex in $X$ and $\omega\in\Omega^*(Y\times X)$, then for each plot $P\colon V\to Y$ of $Y$ and, we have $(P\times\sigma)^*\omega\in\Omega^*(U\times \Delta^n)$ and we can define $$\int_\sigma \omega = \left\{\int_{\Delta^n} (P\times\sigma)^*\omega\right\}$$
    Since $\sigma$ extends smoothly to a small neighborhood of $\Delta^n$ and $\Delta^n$ is compact, the defining intergral converges. The resulting map
    \begin{align*} \int\colon\Omega^d(Y\times X) \otimes C_n^\infty(X)&\to\Omega^{d-n}(Y)\\
        \alpha\otimes \omega&\mapsto \int_\alpha\omega
    \end{align*}
    is the \emph{fiber integral}\index{fiber integral} over smooth singular chains.
\end{construction}

\begin{remark}
    One can make a similar construction with a compact oriented manifold in place of $\sigma$.
\end{remark}
...
For a compact manifold $M$, fix an embedding $M \hookrightarrow \R^D$.

\begin{definition}
The \emphi{Fulton-MacPherson-Axelrod-Singer} compactification of the ordered configuration space of $n$ points in $M$ is the closure of the image of the above map: $\cConf_M(n) = cl_{A_M(n)}(im(\alpha_{M,n}))$. That is, the closure of 
\begin{align*}
    \iota \times \prod_{(i,j)\in \Conf_{\underline n(2)}}\pi_{ij} \times \prod_{(i, j, k)\in\Conf_{\underline n}(3)}s_{ijk} \colon \Conf_M(n)\to M^{n} \times (S^{m-1})^{\Conf_{\underline n}(2)} \times [0, \infty]^{\Conf_{\underline n}(3)}
\end{align*}
The Fulton-MacPherson-Axelrod-Singer compactification of $n$ points in $\R^m$ is $\cConf_m(n) = cl_{A_m(n)}(im(\alpha_{m,n}))$.
\end{definition}

% The maps $\iota, \pi_{ij}, s_{ijk}$ can be extended to the closure $\cConf_M(n)$.

In~\cite{sinha2004manifold} it is shown that if $M$ is a compact manifold, then $\cConf_M(n))$ is a compact manifold with corners, whose top stratum is homeomorphic to $\Conf_M(n)$. In particular, the compactification is homotopy equivalent to the original configuration space. 
...
\begin{remark}
    ...
    The additional elements in the closure correspond to the situation where parts of the tuple become "`infinitesimally close"' as seen from some of the other points, but still have well defined directions with respect to each other. For example in $\cConf_M(3)$ we may have $x\in \cConf_M(3)$ with $\pi_{1,2} = \pi_{1,3}$ and $s_{1,2,3}= 1$, so if we try to restore the original configuration of points from this, the points $x_2$ and $x_3$ are equal. However as an element of the compactification this still has a well-defined $\pi_{2,3}$, i.e.\ a well defined direction between $x_2$ and $x_3$. 

    The relation of being infinitesimally close can be modeled via trees with $n$ leaves and there exists a stratification on $\cConf_M(n)$ where each stratum corresponds to a certain tree, see \cite{sinha2004manifold}. 
\end{remark}
For the $\R^m$ case the base space is not compact and we omit the factor $\iota$ in the definition of $\alpha$, to ensure $\cConf_m(n)$ is compact. 
...
\end{tikzcd}
\end{equation}
is a homotopy pushout since it is a pushout of topological spaces and the map $UTM \to \cConf_M(2)$ is a cofibration. Thus the homotopy cofibers (which can be constructed via the mapping cone) of the  vertical maps are homotopy equivalent due to the pasting law for homotopy pushouts %TODO reference
...
\begin{center}
\begin{tikzcd}
    C^*(TM) \dar["\cupp u"] \rar[equal] & C^*(TM) \dar["\cupp u|_{C^*(TM)} \oplus 0"] & \lar C^*(M)\dar["\cupp e\oplus 0"] \\
    C^{*+n}(TM, T_0M) \rar["\quiso"] & \cone(C^*(TM) \to C^*(T_0M))[n] & \lar["\quiso"] \cone(C^*(M) \to C^*(T_0M))[n]
\end{tikzcd}
\end{center}
...
\end{align*}
\end{definition}
Here $/$ is the homology slant product, which we recall in \cref{subsec:slant_product}. By the remarks in \cref{subsec:fiber-integration}, the slant product is identified with fiber integration through the De Rham isomorphism, hence this construction yields the same map as above. One can show that this map descends to a map $B(C^*(M), C^*(M), C^*(M))\to NC^*(PM)$, where the left side is the simplicial normalization of the bar construction as above and the complex $NC^*(PM)$ is the quotient of $C^*(PM)$ modulo degenerate cochains. 
...
\begin{proposition}\label{thm:path-models-0}
    ...
    \begin{center}
        \begin{tikzcd}
        A \otimes A \rar \dar & \Omega^*(M\times M)\dar \\
        B \rar["I"] & \Omega^*(PM)
        \end{tikzcd}
    \end{center}
...
\begin{center}
\begin{tikzcd}
    \Omega^*(M) & \lar["\int_{\Delta^n}"] \Omega^{*+n}(M\times \Delta^n) & \lar["p^*"]\Omega^{*+n}(M) & \lar\Omega^{*+n}(M) \\
    \Omega^*(PM) \uar["const^*"] &\lar["\int_{\Delta^n}"] \Omega^{*+n}(PM\times\Delta^n) \uar["const^*\otimes\id"] &\lar{ev^*}\uar["\Delta"]\Omega^{*+n}(M^{n+2}) & \lar\Omega^{*+n}(M)^{\otimes (n+2)} \uar["\wedge"]
\end{tikzcd}
\end{center}
...
\begin{theorem}\label{thm:graph-mapping-space}
    ...
    \begin{center}
    \begin{tikzcd}
        A^{\otimes G} \dar \rar & \Omega^*(M^G) \dar \\
        A^{G'}\rar & \Omega^*(M^{G'})
    \end{tikzcd}
    \end{center}
\end{theorem}
\begin{proof}
    ...
    \begin{center}
    \begin{tikzcd}
        M^{\{0, 1\}\times E} & \lar["(s\coprod t)^*"] M^V  \\
...
\begin{theorem}\label{thm:path-models-1}
    ...
    \qquad
    \begin{tikzcd}
        (B\otimes B)\otimes_{A^e\otimes A^e} A\rar["I_{8}"] &\Omega^*(LM\times_M LM) \\
        B\otimes_{A^e}A \uar["\bar c"]\rar["I_\ocircle"] & \Omega^*(LM) \uar["c^*"]
    \end{tikzcd}
    \end{center}
    ...
    \begin{tikzcd}
        \mathcal I \otimes B \rar["\iota_{\mathcal I} \times I_{\ocircle_2}"] & \Omega^*(I\times PM) \\
        B\otimes_A B \rar \uar &\Omega^*(PM\times_M PM) \uar["s^*"]
    \end{tikzcd}
    \qquad
    \begin{tikzcd}
        \mathcal I \otimes (B\otimes_{A^e} A) \rar & \Omega^*(I\otimes LM) \\
...
The left hand side is the upper path of the square, the right hand side is the lower path of the square, up to commuting the factors.
    ...
    \begin{center}
    \begin{tikzcd}
        \Omega^*(PM\times_M PM) \rar\dar[equal] & \mathcal I \otimes \Omega^*(PM) \\
        \Omega^*(PM\times_M PM)\rar["s^*"] & \Omega^*(I\times PM) \uar["\quiso"]
    \end{tikzcd}
    \end{center}
    ...
    \begin{center}
    \begin{tikzcd}
        C^{*-1}(M) \dar[equal] & \lar["p_{\Delta^k\times\Delta^l\times I}^!"] C^{*+k+l}(\Delta^k\times\Delta^l\times I\times M) \dar["(r\times\id)^!"] & \lar["(\Delta\comp(r\times\id))^*"] C^{*+k+l}(M^{1+k+1+l+1}) \dar["\pm\id"]\\
        C^{*-1}(M) & \lar["p_{\Delta^{n+1}}^!"] C^{*+n}(\Delta^{n+1}\times M) &\lar["\Delta^*"] C^{*+n}(M^{1+(n+1)+1})
    \end{tikzcd}
    \end{center}
...
\begin{lemma}
    ...
    \begin{center}
    \begin{tikzcd}
        \Omega^*(LM\times LM) & \lar \Omega^{*}(LM\times_M LM)[-n]  \\
...
\begin{corollary}\label{thm:loop-product-derived}
    ...
    \begin{center}
    \begin{tikzcd}
        \Omega^*(LM)\otimes\Omega^*(LM) & \lar \Omega^*(LM\times_M LM)[-n] & \lar \Omega^*(LM)[-n] \\
        B\otimes_{A^{\otimes 2}} A\otimes B\otimes_{A^{\otimes 2}} A \uar["\quiso"] & \lar["\id\otimes\Delta^!"] B^{\otimes 2}\otimes_{A^{\otimes 4}} A\uar["\quiso"] & B\otimes_{A^{\otimes 2}} A \lar\uar["\quiso"]
    \end{tikzcd}
    \end{center}
...
\begin{definition}
    \begin{enumerate}
        \item An \emph{oriented Poincaré duality algebra}\index{Poincaré duality algebra} of dimension $n$ is a connected CDGA $A$ and a chain map $\epsilon\colon A\to k[-n]$, such that the induced bilinear forms 
    \begin{align*}
        A^k\otimes A^{n-k}\to k, a\otimes b\mapsto \epsilon(ab)
    \end{align*}
    are non-degenerate.
        \item An \emph{oriented differential Poincaré duality algebra} or \emphi{Poincaré CDGA} is a CDGA $A$ together with a chain map $A\to k[n]$ such that $(A, \epsilon)$ is an oriented Poincaré duality algebra and $\epsilon(dA) = 0$.
    \end{enumerate}
\end{definition}
The condition $\epsilon(dA) = 0$ implies that $H^*(A)$ is a Poincaré duality algebra of dimension $n$.

\begin{theorem}\label{thm:lambrechts-stanley-model}
    Let $k$ be a field of any characteristic and let $A$ be a CDGA over $k$ such that $H^*(A)$ is a simply connected oriented Poincaré duality algebra of finite type in dimension $n$. Then there exists a CDGA $A'$ weakly equivalent to $A$ such that $A'$ is a simply connected Poincaré CDGA of finite type.
\end{theorem}
Two CDGAs $A$ and $A'$ are weakly equivalent if there exists a zigzag of quasi-isomorphisms $A\xto{\quiso} \dots\xfrom{\quiso} A'$. In fact, an inspection of the proof in \cite{lambrechts2008poincare} reveals that in the theorem one can take this zigzag to be
\begin{align*}
    A' \xfrom{\quiso} S\xto{\quiso} A
\end{align*}
where $S$ is a minimal Sullivan model of $A$. Moreover, if the orientation and product on $H^*(A)$ come from such structures on $A$, one can take the maps to be morphisms preserving the product and orientation. 
...
\begin{definition}
    A \emphi{differential graded Frobenius algebra} is a unital CDGA $A$ together with a coproduct $\Delta\colon A\to A\otimes A$ of possibly nonzero degree, such that $\Delta$ is a coassociative cocommutative chain map, $\Delta$ is counital, $\Delta$ is a map of left and right $A$-modules and the multiplication on $A$ is a map of left and right $A$-comodules.
\end{definition}
\begin{proposition}
...
\begin{definition}
Let $f\colon M\to  N$ be a map of smooth closed oriented manifolds. Then there is a contravariant \emphi{umkehr map} on homology which we denote by $f_!\colon H_*(N)\to H_{*+m-n}(M)$ which is defined via Poincaré duality as the unique map that makes the following diagram commutative:
\begin{equation}
    \begin{tikzcd}
    ...
    \end{tikzcd}
\end{equation}
More generally, the same diagram on derived chain complexes yields a derived umkehr map $f_! \colon C_*(N) \to C_{*+m-n}(M)$.
\end{definition}
...
\begin{definition}
If $(M, \partial M = A\union B)$ and $(N, \partial N = C \union D)$ are as required for Poincaré duality in \cref{thm:poincare_duality} then a map of pairs $f\colon (M, B) \to (N, D)$ gives rise to a \emphi{relative umkehr map}
\begin{equation}
    \begin{tikzcd}
...
If $M$ is a closed oriented manifold, denote by $\mu_M$ its orientation pairing:
\begin{align*}
...
\begin{proposition}
    The umkehr map $f^!$ of a map $f\colon M\to N$ of closed oriented manifolds is adjoint to $f^*$ in the sense that on derived chain complexes,
    \begin{align*}
...
\begin{definition}
    A \emph{cohomological orientation} of $\xi$ is a family $\{u_b\in H^n(F, F_0)\}_{b\in B}$ such that for every trivialization $E|_U\to U\times \R^n$ and all $b, b'\in U$, the isomorphism $H^n(E|_b, E|_b\setminus 0) \to H^n(E|_U, E|_U\setminus 0) \from H^n(E|_{b'}, E|_{b'}\setminus 0)$ maps $u_b$ to $u_{b'}$.
\end{definition}
If $\xi$ is oriented, one can assign canonically to every fiber $F=\pi^{-1}(b)$ a cohomology class $u_b = u_F\in H^n(F, F_0)$, such that the assignments are locally compatible. Thus an orientation of $\xi$ induces a cohomological orientation with $\Z$-coefficients. Conversely a cohomological orientation with $\Z$-coefficients induces an orientation of $\xi$. 
...
\begin{proof}
    \begin{enumerate}
        ...
        \begin{center}
        \begin{tikzcd}
            H_*(M\times M) \rar \dar["\capp {[\Delta]}^*"] & H_*(M\times M, M\times M\setminus \Delta) \dar["\capp u_\Delta"] &  \lar["\iso"] H_*(N, N_0) \dar["\capp u_N"] \rar["\iso"] & \dar["\capp u"] H_*(DM, D_0M) \\
...
There are two versions of the slant product. We are mostly interested in the product $$/\colon C^n(Y\times X)\otimes C_i(X)\to C^{n-i}(Y).$$ If $X$ is a space with Poincaré duality and with fundamental class $[X]$, then the map $\blank / [X]$ is the umkehr map of the projection $X\times Y\to Y$. At the end we explain the second version of the slant product, which is a map $\\\colon C_n(Y\times X)\otimes C^i(X)\to C_{n-i}(Y)$. 

We follow \cite[chapter 6.1]{spanier1989algebraic}, see there for proofs and details. 
...
$$\langle c^*/c', c\rangle = \langle c^*, EZ(c', c)\rangle$$
...
% \end{align*}
where $EZ\colon C_*(Y)\otimes C_*(X)\to C_*(Y\times X)$ is the Eilenberg Zilber map.
\end{definition}
