...
\begin{align*}
    ...
    I(\omega_0 \otimes \dots \otimes\omega_{n+1}) &= (-1)^{\zeta}\int_{\Delta^n}\phantom{|} ev_n^*(\pi_0^*\omega_0\wedge\dots\wedge\pi_{n+1}^*\omega_{n+1})
\end{align*}
...
where $\zeta = \abs{\omega_1}+2\abs{\omega_2}+\dots+(n+1)\omega_{n+1} + n(n+1)/2$. It acts on each summand up to sign as the composition
\begin{align*}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        % =& \sum_{i=0}^{n+1} (-1)^{\abs\omega_0+\dots+\abs\omega_{i-1}} \int\omega_0\otimes\dots d\omega_i\dots\otimes\omega_{n+1}\\
...
\begin{theorem}
    ...
    \begin{tikzcd}[row sep=.27em, column sep=2cm]
        ...
        B\otimes_{A^e} B\otimes_{A^e} A \rar["\int\otimes \int"] &\Omega^*(\Map(8, M))
    \end{tikzcd}
\end{center}
